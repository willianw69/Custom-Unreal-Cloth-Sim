...
FClothMeshSceneProxy::FClothMeshSceneProxy(
	UMeshComponent* Component,
	const TArray<FDynamicMeshVertex>& InVertices,
	const TArray<uint32>& InIndices,
	UMaterialInterface* InMaterial)
	: FPrimitiveSceneProxy(Component)
	, VertexFactory(GetScene().GetFeatureLevel(), "FClothMeshSceneProxy")
	, Material(InMaterial)
...
SIZE_T FClothMeshSceneProxy::GetTypeHash() const
{
	static size_t UniquePointer;
	return reinterpret_cast<size_t>(&UniquePointer);
}

void FClothMeshSceneProxy::UpdateVertices_RenderThread(
...
void FClothMeshSceneProxy::GetDynamicMeshElements(
...
FPrimitiveViewRelevance FClothMeshSceneProxy::GetViewRelevance(const FSceneView* View) const
{
...
bool FClothMeshSceneProxy::CanBeOccluded() const
{
	return !MaterialRelevance.bDisableDepthTest;
}

uint32 FClothMeshSceneProxy::GetMemoryFootprint() const
{
