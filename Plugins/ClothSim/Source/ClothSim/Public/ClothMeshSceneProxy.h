...
/**
 * Scene proxy for the cloth mesh.
 *
 * Modeled on the engine's FProceduralMeshSceneProxy (single section). Topology is
 * static (the grid never re-triangulates), so the index buffer is built once. Each
 * frame the component pushes fresh vertex positions/normals from the GPU sim
 * (delivered via a small CPU readback) and we update the position + tangent vertex
